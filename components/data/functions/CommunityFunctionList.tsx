...
const CommunityFunctionsList: React.FC<CommunityFunctionsListProps> = (props) => {
  ...
  const handleClone = async (functionId: string) => {
    ...
    return await cloneFunction(functionId).then((result) => {
      if (result.status !== 'ok') {
        throw new Error(result.message);
      }
      const toastProps = {
              ...
              <>
                ...
                <div className="w-full"></div>
                {
                  functionRecord.isSubscribed ? (
                    <Button onClick={() => handleSubscribeToggle(functionRecord.id)} disabled={isSubscribing} type='button' variant='outline' aria-label='Unsubscribe' title='Unsubscribe' className="text-xs px-3 py-2 h-auto rounded-3xl text-destructive">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Are you sure to subscribe &quot;{functionRecord.slug}&quot; from {functionRecord.owner.profile?.userName}?</AlertDialogTitle>
                          <AlertDialogDescription>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => handleSubscribeToggle(functionRecord.id)} disabled={isSubscribing}>Continue</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  )
