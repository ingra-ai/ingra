...
export const handleFetch = async (
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to run handleFetch with status ${response.status}: ${response.statusText}`,
      );
    }

    return await response.json();
  } catch (error: any) {
    Logger.withTag("handleFetch").error(
      `Failed to run fetch: ${error?.message || "Unknown Error"}`,
      { url, method },
    );
    throw error;
  }
