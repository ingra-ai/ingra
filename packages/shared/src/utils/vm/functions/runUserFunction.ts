...
export const runUserFunction = async (
  authSession: AuthSessionResponse,
  functionRecord: RunUserFunctionArgType,
  ...
  const vmContext = generateVmContextArgs(
    authSession,
  ...
  const errors = vmOutput.outputs.filter(
      (output) => output.type === "error",
    ) as UserSandboxOutput[],
    metrics = (
      (vmOutput.outputs || []).filter(
        (output) => output.type === "metric",
      ) as MetricSandboxOutput[]
    ).reduce<Partial<MetricValues>>((acc, metric) => {
