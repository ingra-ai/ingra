...
export const EnvVarsTable: FC<EnvVarsTableProps> = (props) => {
  ...
  const withoutDateColumns = envVars.every(
    (envVar) =>
      envVar.createdAt === undefined || envVar.updatedAt === undefined,
  );
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm">
            Your environment variables will be passed as part of{" "}
            <code className="text-orange-500 italic font-semibold">ctx</code> in
            your functions
          </p>
        </div>
      ...
      <table className="mt-4 w-full whitespace-nowrap text-left table-fixed">
        ...
        <thead className="border-b border-white/10 text-sm leading-6">
          <tr>
            ...
            <th
              scope="col"
              className="pl-3 py-3.5 text-left text-sm font-semibold w-full"
            >
              Value
            </th>
            {!withoutDateColumns && (
              <>
                <th
                  scope="col"
                  className="px-3 py-3.5 text-left text-sm font-semibold truncate"
                >
                  Created At
                </th>
                <th
        ...
        <tbody>
          {envVars.map((envVar) => {
            return (
              <tr key={envVar.id}>
                <td
                  className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium sm:pl-0 truncate"
                  title={envVar.key}
                >
                  <code className="text-orange-500 italic font-semibold">
                    {envVar.key}
                  </code>
                </td>
                ...
                <td className="relative whitespace-nowrap py-4 pr-4 text-right text-sm font-medium sm:pr-0">
                  <button
                    ...
                    aria-label="Edit"
                    title="Edit"
                    className="p-1 mr-2 text-zinc-300 hover:text-zinc-400"
                  >
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          Are you sure to delete &quot;{envVar.key}&quot;?
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                          This action cannot be undone. This will permanently
                          delete your environment variable.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => onDelete(envVar)}>
                          Continue
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
