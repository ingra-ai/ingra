...
} from "@repo/shared/actions/collections";
...
import type { CommunityCollectionListGetPayload } from "../../../data/collections/community/types";
...
interface CommunityCollectionListProps
  extends React.HTMLAttributes<HTMLDivElement> {
  showControls: boolean;
  collections: CommunityCollectionListGetPayload[];
}

const CommunityCollectionList: React.FC<CommunityCollectionListProps> = (
  props,
) => {
  const { showControls = false, collections, ...divProps } = props;
  const router = useRouter();
  const [isSubscribeLoading, setIsSubscribeLoading] = useState<false | string>(
    false,
  );
    ...
    return subscribeToCollection(collection.id)
      .then((result) => {
        ...
        toast({
          title: "Success!",
          description:
            result?.message || "Collection has been subscribed successfully.",
        });
  ...
  const handleUnsubscribe = (collection: CommunityCollectionListGetPayload) => {
    ...
    const confirmed = confirm(
      `Are you sure to unsubscribe to this collection? Unsubscribing will remove all the functions in it accessible from you.`,
    );
  ...
  return (
    <div
      data-testid="community-collection-list"
      {...divProps}
      className={classes}
    >
        ...
        <div className={collectionListGridClasses}>
          {collections.map((collection) => {
            const isSubscribing = isSubscribeLoading === collection.id,
              href = getUserRepoCollectionsViewUri(
                collection.owner.profile?.userName || "",
                collection.id,
              );
                      ...
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button
                            ...
                            title="Subscribe"
                            className="text-xs px-3 py-2 h-auto rounded-3xl"
                          >
                            {isSubscribing ? (
                              <RefreshCcw className="w-4 h-4 animate-spin inline-block mr-2" />
                            ) : (
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>
                              Are you sure to subscribe &quot;{collection.slug}
                              &quot; from {collection.owner.profile?.userName}?
                            </AlertDialogTitle>
                            <AlertDialogDescription>
                              Subscribing to this collection allows you to
                              subscribe to all of its own functions as well.
                              However, please note that you won&apos;t be able
                              to customize or modify the functions in it.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
