...
import {
  cloneFunction,
  collectionToggleFunction,
  upsertFunction,
} from "@repo/shared/actions/functions";
...
export const FunctionForm: FC<FunctionFormProps> = (props) => {
  ...
  const allUserAndEnvKeys = Object.keys(userVars).concat(
  ...
  const methods = useForm<z.infer<typeof FunctionSchema>>({
      ...
      defaultValues: {
        ...
        code:
          functionRecord?.code ||
          generateCodeDefaultTemplate(allUserAndEnvKeys),
        isPrivate:
          typeof functionRecord?.isPrivate === "undefined"
            ? true
            : !!functionRecord?.isPrivate,
        isPublished:
          typeof functionRecord?.isPublished === "undefined"
            ? false
            : !!functionRecord?.isPublished,
        ...
        .then((resp) => {
          ...
          if (resp?.data?.id) {
            if (isEditMode) {
              ...
              redirectUrl = getUserRepoFunctionsEditUri(
                ownerUsername,
                resp.data.id,
              );
            }
    ...
    const clonedFunction = await cloneFunction(functionId)
      .then((result) => {
        ...
        if (functionHref) {
          toastProps.action = (
            <ToastAction
              altText="Cloned Function"
              onClick={() => router.replace(functionHref)}
            >
  ...
  const onFunctionCollectionToggleChanged = (
    collectionId: string,
    functionId: string,
    checked: boolean,
  ) => {
        ...
        >
          <div className="block space-y-6 col-span-1 order-2 lg:order-1 md:col-span-6 xl:col-span-7 2xl:col-span-8">
            <div className="block">
              ...
              />
              {errors.code && (
                <p className="text-sm font-medium text-destructive-foreground mt-3">
                  {errors.code.message}
                </p>
              )}
            </div>
          ...
          <Tabs
            id="function-form-tabs"
            defaultValue="function-properties-tab"
            className="block col-span-1 order-1 lg:order-2 lg:col-span-6 xl:col-span-5 2xl:col-span-4"
          >
            <div className="flex justify-between mb-4">
              <TabsList className="">
                <TabsTrigger value="function-properties-tab">
                  Properties
                </TabsTrigger>
              ...
              {functionRecord && (
                <div className="block">
                  <Button
                    ...
                    type="button"
                    disabled={isSaving}
                    className="flex justify-center rounded-md text-sm font-semibold"
                    onClick={() => setSandboxOpen(true)}
                  >
                    <BugPlayIcon className="inline-block mr-2 w-4 h-4" /> Dry
                    Run
                  </Button>
                </div>
              )}
                      ...
                      <div className="flex flex-row items-center space-x-3 space-y-0 px-2 py-2">
                        <div className="flex flex-col w-full space-y-1 leading-none">
                          ...
                          <p className="text-xs font-medium text-muted-foreground mt-3">
                            Toggle this option to publish or unpublish the
                            function. Published functions are available for use,
                            while unpublished functions remain in draft mode and
                            are not accessible.
                          </p>
                        </div>
                    ...
                    return (
                      <div className="flex flex-row items-center space-x-3 space-y-0 px-2 py-2">
                        <div className="flex flex-col w-full space-y-1 leading-none">
                          ...
                          <p className="text-xs font-medium text-muted-foreground mt-3">
                            Unchecking this will share your function to
                            Marketplace. Only works for published functions.
                          </p>
                        </div>
                        <div className="flex justify-end">
                          <Switch
