...
const FunctionsList: React.FC<FunctionsListProps> = ({
  ...
  const handleDelete = (functionId: string) => {
    ...
    const confirmed = confirm(
      `Are you sure to delete function? This action cannot be undone. This will permanently delete your function.`,
    );
      ...
      {functions.map((functionData) => {
        return (
          <FunctionItem
            ...
            href={getUserRepoFunctionsEditUri(ownerUsername, functionData.id)}
          >
            <div className="flex mt-4 bg-card border rounded-[10px] px-3 py-2">
              ...
              <button
                type="button"
                onClick={() => handleEdit(functionData.id)}
                aria-label="Edit"
                title="Edit"
                className="text-info hover:text-info-foreground p-2 hover:bg-accent"
              >
                <PencilIcon className="h-4 w-4" />
              </button>
              <button
                type="button"
                onClick={() => handleDelete(functionData.id)}
                aria-label="Delete"
                title="Delete"
                className="text-destructive hover:text-destructive-foreground p-2 hover:bg-accent"
              >
