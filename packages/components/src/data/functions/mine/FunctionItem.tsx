...
const FunctionItem: React.FC<React.PropsWithChildren<FunctionItemProps>> = (
  props,
) => {
  ...
  return (
    <div className="flex flex-col bg-secondary border border-gray-500 hover:border-gray-300 shadow-md rounded-sm px-4 py-2 w-full min-h-[200px]">
      <Link
        className="block space-y-2 py-2 h-full cursor-pointer"
        role="button"
        tabIndex={0}
        aria-pressed="false"
        href={href}
      ...
      >
        ...
        <h2
          ...
          title={functionData.slug}
        >
          {functionData.slug}
        </h2>
        <span className="block">
          <span
            className={`px-2 py-1 inline-flex text-xs leading-3 font-semibold rounded-full ${functionData.isPrivate ? "bg-red-700 text-red-200" : "bg-green-700 text-green-200"}`}
          >
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p className="truncate min-w-0">
                  Last update on {format(functionData.updatedAt, "PPpp")}{" "}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
