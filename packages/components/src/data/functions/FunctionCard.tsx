...
export const FunctionCard: React.FC<React.PropsWithChildren<FunctionCardProps>> = (props) => {
    ...
    <div className="flex flex-col bg-secondary border border-gray-500 hover:border-gray-300 shadow-md rounded-sm px-4 py-2 w-full min-h-[200px]">
      <Link className="block space-y-2 py-2 h-full cursor-pointer" role="button" tabIndex={0} aria-pressed="false" href={href} title={functionData.description}>
        ...
        <span className="block">
          <span className={`px-2 py-1 inline-flex text-xs leading-3 font-semibold rounded-full ${functionData.isPrivate ? 'bg-red-700 text-red-200' : 'bg-green-700 text-green-200'}`}>{functionData.isPrivate ? 'Private' : 'Public'}</span>
...
interface FunctionCardNewProps {
  ownerUsername: string;
}

export const FunctionCardNew: React.FC<FunctionCardNewProps> = ({ ownerUsername }) => {
  return (
    <Link href={getUserRepoFunctionsNewUri(ownerUsername)} className="flex flex-col justify-center items-center text-gray-500 hover:text-gray-300 border border-gray-500 hover:border-gray-300 shadow-md rounded-sm p-4 w-full min-h-[200px]">
