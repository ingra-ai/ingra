    ...
    if ( sessionWithUser?.user ) {
      ...
      if ( user.id && user.email && user.oauthTokens.length ) {
        ...
        if ( !defaultOAuthToken ) {
          Logger.withTag('getAuthSession').withTag(`user:${ user.id }`).error('User has no default OAuth token.');
        ...
        const newOAuthCredentials = await refreshGoogleOAuthCredentials(defaultOAuthToken).then( async (newOAuth) => {
          if ( newOAuth?.credentials ) {
            ...
            const updatedOAuthRecord = updateOAuthToken(newOAuth.userId, newOAuth.primaryEmailAddress, newOAuth.credentials).then( (updatedOAuth) => {
              if ( updatedOAuth ) {
                Logger.withTag('getAuthSession').withTag(`user:${ user.id }`).info('Refreshed OAuth tokens:', { 
           ...
           */
          Logger.withTag('getAuthSession').withTag(`user:${ user.id }`).error('Error refreshing OAuth credentials:', err);
          Logger.withTag('getAuthSession').withTag(`user:${ user.id }`).error('Deleting oauth token due to refresh token may be corrupted.', { id: defaultOAuthToken?.id });
