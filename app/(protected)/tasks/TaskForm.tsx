...
export const TaskForm: React.FC<TaskFormProps> = (props) => {
  ...
  const onSubmit = useCallback((values: z.infer<typeof TaskSchema>) => {
    setIsLoading(true);
    return createTask(values).then((data) => {
      toast({
        title: "Task created!",
      ...
      <div>
        ...
        />
        {
          formState.errors.title && (
            <p className="text-sm font-medium text-destructive-foreground mt-3 text-center">{formState.errors.title.message}</p>
          )
        }
      </div>
      <div>
        ...
        />
        {
          formState.errors.description && (
            <p className="text-sm font-medium text-destructive-foreground mt-3 text-center">{formState.errors.description.message}</p>
          )
        }
      </div>
      <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        <div className="sm:col-span-3">
          <div>
            ...
            <Select
              onValueChange={(value) => setValue('priority', TaskPriority.parse(value), { shouldValidate: true })}
              defaultValue={TaskPriority.enum.MEDIUM}>
              <SelectTrigger>
        ...
        <div className="sm:col-span-3">
          <div>
            ...
            <Select
              onValueChange={(value) => setValue('status', TaskStatus.parse(value), { shouldValidate: true })}
              defaultValue={TaskStatus.enum.TODO}>
              <SelectTrigger>
