...
import { collectionToggleFunction, deleteFunction } from '@protected/functions/actions/functions';
import { PencilIcon, TrashIcon } from '@heroicons/react/24/outline';
import type { FunctionListGetPayload, CollectionListGetPayload } from '@protected/functions/mine/types';
...
const FunctionsList: React.FC<FunctionsListProps> = ({ functions, collections }) => {
  ...
  const handleEdit = (functionId: string) => {
    router.push(`/functions/edit/${functionId}`);
  };
  ...
  return (
    <>
      ...
      {functions.map((functionData) => {
        return (
          <FunctionItem key={functionData.id} functionData={functionData} href={`/functions/edit/${functionData.id}`}>
