...
import { RefreshCcw, ListChecksIcon } from 'lucide-react';
...
import { CollectionListGetPayload } from '@protected/collections/mine/types';

type CollectionFormProps = {
  collectionRecord?: CollectionListGetPayload;
  onCancel: () => void;
};
    ...
    if (collectionRecord?.id) {
      ...
      return await updateCollection(collectionRecord.id, values).then((resp) => {
        if (resp?.data?.id) {
          toast({
            ...
            action: (
              <ToastAction altText="My Collections" onClick={() => router.replace(`/collections/mine`)}>
                <ListChecksIcon className="w-3 h-3 mr-3" /> My Collections
              </ToastAction>
            )
          });
        }
      ...
      return await createCollection(values).then((resp) => {
        if (resp?.data?.id) {
          ...
          startTransition(() => {
            router.push(`/collections/edit/${resp.data.id}`);
          });
        }
      })
      .catch((error: Error) => {
