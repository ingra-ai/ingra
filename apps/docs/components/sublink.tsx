...
export default function SubLink({ title, href, items, noLink, level, isSheet }: EachRoute & { level: number; isSheet: boolean }) {
  ...
  const titleOrLink = !noLink ? isSheet ? <SheetClose asChild>{Comp}</SheetClose> : Comp : <h4 className="font-medium sm:text-sm text-primary">{title}</h4>;
        ...
        <div className="flex items-center gap-2">
          ...
          <CollapsibleTrigger asChild>
            <Button className="ml-auto mr-3.5 h-6 w-6" variant="link" size="icon">
              {!isOpen ? <ChevronRight className="h-[0.9rem] w-[0.9rem]" /> : <ChevronDown className="h-[0.9rem] w-[0.9rem]" />}
              <span className="sr-only">Toggle</span>
            </Button>
          </CollapsibleTrigger>
        </div>
        <CollapsibleContent>
          <div className={cn('flex flex-col items-start sm:text-sm dark:text-neutral-300/85 text-neutral-800 ml-0.5 mt-2.5 gap-3', level > 0 && 'pl-4 border-l ml-1')}>
            {items?.map((innerLink) => {
              const modifiedItems = {
                ...innerLink,
