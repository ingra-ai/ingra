...
<Stepper>
  <StepperItem title="Run Flask Server in your local machine">
    ...
    <Note title="Bearer Token" type="info">
      Ingra's [flask_server.py]({CONST.APP_GITHUB_URL}/blob/main/local-scripts/flask_server.py) require a bearer token which will be used at later stage.
    </Note>

  </StepperItem>
  <StepperItem title="Run Ngrok in your local machine">
    Setting up ngrok is simple and free, follow their guide [at their Download page](https://ngrok.com/download)

    Run ngrok and provide the Flask server `PORT`. In my case, it's `5000`
  ...
  <StepperItem title="Saving to Ingra Environment Variables">
    ...
    You can add it at your [Environment Variables at Ingra]({CONST.HUBS_APP_URL}{CONST.APP_SETTINGS_ENV_VARS_URI})

    #### Via GPT (ChatGPT)

    To setup ChatGPT with Ingra, you can follow our guide here [Setting up Ingra for Custom ChatGPT](/docs/getting-started/quick-start-guide/chatgpt-setup)
  ...
  <StepperItem title="Create the function at Ingra">
    You have 3 options:
  ...
  <StepperItem title="Invoke the function from GPT">
    Once you have the function, you can invoke it directly from ChatGPT by searching the function first. Since you already have [Built-in function](/docs/features/builtin-functions)
    ...
    <Note title="Updating openapi.json" type="note">
      Optionally, after you own the function - you can visit your [openapi.json]({CONST.HUBS_OPENAPI_URL}) and update to your own Custom ChatGPT.

      Read more about that at [Setting up Ingra for Custom ChatGPT](/docs/getting-started/quick-start-guide/chatgpt-setup)
    </Note>
  </StepperItem>
