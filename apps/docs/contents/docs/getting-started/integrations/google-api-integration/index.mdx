...
Ingra requires specific permissions to Google services to manage and streamline various tasks effectively. This guide outlines the necessary Google API scopes and their purposes.

<Note title="Important" type="note">
  Some of these permissions are required to run some functions that are available at marketplace.
</Note>
