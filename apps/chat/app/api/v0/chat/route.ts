...
import { LangChainAdapter, Message as VercelChatMessage, streamText } from 'ai';
...
export async function POST(req: NextRequest) {
  ...
  if ( !authSession ) {
    ...
    const readableStream = new ReadableStream({
      ...
      controller.enqueue(textEncoder.encode(unauthorizedMessage));
      controller.close();
      },
    });
  ...
  return new Response(LangChainAdapter.toDataStream(readableStream));
}
