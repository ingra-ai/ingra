...
const getApiDetails = async (
  ...
  if (
    !authSwaggerSpec ||
    typeof authSwaggerSpec?.paths !== "object" ||
    !toolName
  ) {
    return null;
  }

  for (const [path, pathSchema] of Object.entries<Record<string, any>>(
    authSwaggerSpec.paths,
  )) {
    for (const [httpVerb, details] of Object.entries<Record<string, any>>(
      pathSchema,
    )) {
...
export const actionToolCalls = async (
  ...
  const resultPromises = toolCalls.map(async (toolCall) => {
    const logger = Logger.withTag("actionToolCalls")
      .withTag(`thread|${threadId}`)
    ...
    try {
      ...
      const toolOutput = await toolsHandler(
        authSession,
        functionName,
        functionArgs,
        headers,
      ...
      logger.error(
        `Error handling tool call: ${err?.message || "Unknown Error"}`,
      );
