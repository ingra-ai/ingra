...
export const MagicLoginForm: FC<MagicLoginFormProps> = (props) => {
  ...
  const { handleSubmit, register, formState, setValue, reset } = useForm<
    z.infer<typeof MagicLoginSchema>
  >({
  ...
  if (formView === "redirect") {
    return (
      <div className={classes} data-testid="magic-login-form">
        <h2 className="text-center text-sm font-bold leading-9 tracking-tight">
          Signing You In
        </h2>
        ...
        <p className="my-5 text-center text-sm text-white/70 max-w leading-6">
          Please hold on a moment. We&lsquo;re currently working on signing you
          into your account. This should only take a few seconds. If it&lsquo;s
  ...
  return (
    <div className={classes} data-testid="magic-login-form">
      {formView === "email" ? (
        <p className="mt-2 text-left text-sm text-white/70 max-w">
          To get started, we will need to send you a magic link and one-time
        ...
        <>
          ...
          <p className="mt-2 text-center text-sm text-white/70 max-w">
            We’ve sent a 6-character code to your email address. The code
            expires shortly, so please enter it soon.
          </p>
        </>
      )}
      <form
        className="block space-y-6 mt-10"
        method="POST"
        onSubmit={handleSubmit(doMagicLogin)}
        autoComplete="off"
      >
        {formView === "email" ? (
          <div>
            ...
            {formState.errors.email && (
              <p className="text-sm font-medium text-destructive-foreground mt-3 text-center">
          ...
          <div>
              ...
              render={({ slots }) => (
                <InputOTPGroup className="mx-auto">
                  {slots.map((slot, index) => (
                    <InputOTPSlot
                      key={index}
                      className="rounded-md border text-xl mx-1 h-10 w-10 xl:h-15 xl:w-15 xl:mx-2 bg-gray-950"
                      {...slot}
                    />
                  ))}{" "}
                </InputOTPGroup>
            ...
            {formState.errors.otpCode && (
              <p className="text-sm font-medium text-destructive-foreground mt-3 text-center">
                {formState.errors.otpCode.message}
              </p>
            )}
          </div>
        )}
