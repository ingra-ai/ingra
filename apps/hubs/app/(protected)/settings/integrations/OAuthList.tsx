    ...
    setTokenAsDefault(token)
      .then((result) => {
        ...
        toast({
          ...
          description:
            result.message || `Token has been successfully set as default.`,
        });
    ...
    <ul role="list" className="grid grid-cols-1 gap-6 xl:grid-cols-2">
      {oAuthTokens.map((token) => {
        ...
        return (
          <li
            key={token.id}
            className="col-span-1 divide-y divide-gray-500 rounded-lg border border-gray-600 bg-secondary text-secondary-foreground"
          >
            <div className="flex w-full items-center justify-between space-x-6 p-6">
              <div className="flex-1 truncate">
                ...
                <p className="mt-2 truncate text-sm text-gray-500">
                  {token.primaryEmailAddress}
                </p>
                <p className="mt-1 truncate text-xs text-gray-400">
                  Created at: {format(token.createdAt, "yyyy-MM-dd")}
                </p>
              </div>
              {tokenDetail.icon}
            </div>
                ...
                <div className="flex w-0 flex-1">
                  ...
                  >
                    {isTokenLoading ? (
                      ...
                      <TentIcon className="h-4 w-4 mr-2" aria-hidden="true" />
                    )}
                    {isTokenLoading
                      ? "Setting as default..."
                      : "Set as Default"}
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription className="py-4 space-y-4 leading-6">
                          <p>
                            Your privacy is paramount to us, and we handle it
                            with the utmost seriousness. Should you choose to
                            revoke access, re-authorization of our application
                            for your account access will be necessary.
                          </p>

                          <p>
                            For enhanced security, we recommend visiting your
                            Google Account&apos;s connections section before
                            proceeding with this step. This allows you to remove{" "}
                            <strong>{APP_NAME}</strong> from your list of
                            connected applications directly.
                          </p>
