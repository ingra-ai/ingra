...
export const OpenAPISpecLoader: React.FC<OpenAPISpecLoaderProps> = (props) => {
  ...
  const onChecked = useCallback(
    (checkedKeys: React.Key[] | { checked: React.Key[]; halfChecked: React.Key[] }) => {
      ...
      setRefinedSpec({
        ...spec,
        paths: Object.fromEntries(Object.entries(spec.paths).filter(([path]) => checkedPaths.includes(path))),
      });
    },
    [spec]
  );
    ...
    <div className={classes} data-testid="openapi-spec-loader">
      ...
      <div className="flex-1 overflow-hidden">
        <div className="flex justify-end space-x-4">
          <Button type="button" variant="accent" className="rounded bg-transparent border" title="Copy OpenAPI spec to clipboard" onClick={handleCopy}>
            <Copy className="w-4 h-4" />
            <span className="sr-only">Copy</span>
          </Button>
        </div>
        <ScrollArea className="h-[calc(100%-60px)]">
          <pre className="p-4 rounded">{JSON.stringify(refinedSpec, null, 2)}</pre>
        </ScrollArea>
      </div>
    </div>
  );
