  ...
  const sideNavRoutes: NavItem[] = [
    {
      name: "Dashboard",
      description:
        "Provides a summary of user activities, including usage metrics of various utilities and services.",
    ...
    // },
    {
      name: "Hubs",
      description:
        "Dynamic hubs that your AI can utilize, including collections, functions, subscriptions, workflows and more.",
      children: [
        {
          name: "Marketplace",
          description:
            "Browse public collections and functions shared by other users.",
        ...
        },
        {
          name: "Repository",
          description:
            "Manage your collections and functions that you own or have access to.",
...
const SideNav: FC<SideNavProps> = (props) => {
  ...
  const [censoredUser, censoredEmail] = censorEmail(
    authSession?.user?.email || "unknown@unknown.com",
  );
  const pathname = usePathname();
  const sideNavRoutes = getSideNavRoutes(authSession);

  // Type guard to determine if a NavItem is a NavItemParent
  const isNavItemParent = useCallback(
    (item: NavItem): item is NavItemParent => {
                ...
                <li key={item.name}>
                  {isParent ? (
                    <div className={cn("block", { "mt-8": hasChildren })}>
                      <div
                        className="flex items-center text-xs font-semibold leading-6 text-gray-400"
                  ...
                  <MenuItems className="absolute bg-gray-900 border border-gray-700 right-0 mt-2 w-48 origin-top-right rounded-md py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none bottom-full">
                    <MenuSection>
                      <MenuHeading className="px-4 pt-3">
                        ...
                        <p className="text-xs font-semibold leading-8">
                          {authSession?.user?.email}
                        </p>
                      </MenuHeading>
                      ...
                      {SettingsNavRoutes.map((item) => (
                        <MenuItem key={item.name}>
                          <Link
                            href={item.href}
                            className="flex font-semibold space-x-2 px-4 py-3 text-xs hover:bg-gray-700"
                            title={item.name}
                          >
                    ...
                    <MenuItem>
                      <a
                        href="/auth/logout"
                        className="flex font-semibold space-x-2 px-4 py-3 text-xs text-destructive hover:bg-gray-700 hover:text-destructive-foreground"
                        title="Logout"
                      >
